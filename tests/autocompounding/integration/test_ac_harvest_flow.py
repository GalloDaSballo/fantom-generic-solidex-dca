...
from helpers.SnapshotManagerEmittingBasic import SnapshotManager
...
def test_single_user_harvest_flow_remove_fees(deployer, reward, vault, strategy, want, keeper):
    # Setup
    randomUser = accounts[6]
    snap = SnapshotManager(vault, strategy, "StrategySnapshot")
    startingBalance = want.balanceOf(deployer)
    ...
    ## If the strategy is printing, this should be true // Shares issued from Perf Fee
    assert vault.balanceOf(vault.treasury()) > 0
    assert vault.vault.getPricePerFullShare() > 1e18
